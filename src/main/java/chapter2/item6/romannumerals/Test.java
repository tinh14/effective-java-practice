...
public class Test {
    public static void main(String[] args) throws Exception {
        RomanNumerals dumbRomanNumerals = new DumbRomanNumerals();
        ...
        for (int i = 0; i < numSets; i++) {
            ...
            totalDumbTime += elapsed;
            System.out.println("Smart run #" + (i + 1) + ": " + (elapsed / 1_000_000.0) + " ms");
        }

        System.out.println("=== Total Smart time ===");
        System.out.println((totalDumbTime / 1_000_000_000.0) + " s total for all " + numSets + " runs");

        System.out.println("=== Dumb version ===");
        for (int i = 0; i < numSets; i++) {
            long start = System.nanoTime();
            for (int j = 0; j < numReps; j++) {
                b ^= dumbRomanNumerals.isRomanNumeral("MCMLXXVI");
            }
            long end = System.nanoTime();
            long elapsed = end - start;
            ...
            System.out.println("Dumb run #" + (i + 1) + ": " + (elapsed / 1_000_000.0) + " ms");
        }

        System.out.println("=== Total Dumb time ===");
